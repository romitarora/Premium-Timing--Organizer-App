...
- (void)updateIndicators {
	...
	else if(mode == MBProgressHUDModeHidden)
	{
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.detailsLabelFont =  [UIFont systemFontOfSize:12];
        self.xOffset = 0.0f;
        self.yOffset = 0.0f;
...
{
    if(self.mode == MBProgressHUDModeHidden) {
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
		...
		//take label up when activity indicator is set hidden
		if (mode == MBProgressHUDModeHidden)		
		{
			CGRect frmWithoutActivityIndicator = label.frame;
			frmWithoutActivityIndicator.origin.y = frmWithoutActivityIndicator.origin.y-18;
			
			if ([self.labelText length] >42)
			{
				NSLog(@"labelText length = %d",[self.labelText length]);
				NSLog(@"lenght/25 = %.f",round([self.labelText length]/25.0));
				int numberofLines = round([self.labelText length]/25.0);
				label.numberOfLines = numberofLines;
				NSLog(@"numberof lines = %d",numberofLines);
				frmWithoutActivityIndicator.size.height = frmWithoutActivityIndicator.size.height + (numberofLines *20);
				
				frmWithoutActivityIndicator.origin.y = frmWithoutActivityIndicator.origin.y-(10*numberofLines);
				
				self.height = self.height + PADDING*numberofLines;
				NSLog(@"hiegh21 = %f",self.height);
			}
			else {
				self.height = lHeight + (lHeight/2);
			}

			label.frame = frmWithoutActivityIndicator;
		}
		else {
			self.height = self.height + lHeight + PADDING;
		}
		...
		if (viewPosition == MBProgressHUDPositionBottom)
		{
			CGRect frmWithoutActivityIndicator = self.frame;
//			frmWithoutActivityIndicator.origin.y = frmWithoutActivityIndicator.origin.y+78;
			frmWithoutActivityIndicator.origin.y = frmWithoutActivityIndicator.origin.y+138;			
			self.frame = frmWithoutActivityIndicator;	
		}
...
//	NSLog(@"%s \ncount = %d",__FUNCTION__,self.showCount);
...
- (void)done {
	...
	if (removeFromSuperViewOnHide) {
		[self removeFromSuperview];	//uncomment this line if app crashes while loader is displaying. and,
		//self = nil;						//comment this line, Shreya
		//[self release];					//commnet this line, Shreya
	}
}
...
- (void)drawRect:(CGRect)rect
{
	...
	//	CGContextSetRGBFillColor(context, 1.0, 0.64, 0.11, 1.0);
...
- (void)setTransformForCurrentOrientation:(BOOL)animated {
	...
	if (UIInterfaceOrientationIsLandscape(orientation))
    {
        if (IS_OS_8_OR_LATER)
        {
            if (orientation == UIInterfaceOrientationLandscapeLeft) { degrees = 360; }
            else { degrees = 90; }
        }
        else
        {
